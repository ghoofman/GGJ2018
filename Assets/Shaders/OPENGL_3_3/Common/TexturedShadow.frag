...
float ShadowCalculation(vec4 fragPosLightSpace)
{
    ...
    float bias = max(0.0000005 * (1.0 - dot(normal, lightDir)), 0.00000005);
...
void main()
{
	vec4 texColor = texture(uAlbedoMap, fs_in.TexCoords);
	...
	if(fs_in.FragPos.z < -20) {
		behindZ = -1 * max(-1.0, (fs_in.FragPos.z - 20) / 15.0);
	}
    ...
    FragColor = vec4(finalColor, texColor.a) * (1.0 - belowY) * (1.0 - behindZ);
}
