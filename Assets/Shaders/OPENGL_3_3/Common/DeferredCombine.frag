...
vec3 texColor(vec2 uv) {
	vec3 result = texture(uGbufferAlbedoSpec, uv).rgb;
	float dist = 0.00025;
	result += texture(uGbufferAlbedoSpec, uv + vec2(0.0, dist)).rgb;
	result += texture(uGbufferAlbedoSpec, uv + vec2(0.0, -dist)).rgb;
	result += texture(uGbufferAlbedoSpec, uv + vec2(dist, 0.0)).rgb;
	result += texture(uGbufferAlbedoSpec, uv + vec2(-dist, 0.0)).rgb;
	result += texture(uGbufferAlbedoSpec, uv + vec2(-dist, -dist)).rgb;
	result += texture(uGbufferAlbedoSpec, uv + vec2(dist, dist)).rgb;
	result += texture(uGbufferAlbedoSpec, uv + vec2(-dist, dist)).rgb;
	result += texture(uGbufferAlbedoSpec, uv + vec2(dist, -dist)).rgb;
	return result / 9.0;
...
{             
    ...
    vec3 Diffuse = texColor(tex_coord);
